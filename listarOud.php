...
$aleatorio = new NumeroAleatorio($numLoteria,$numParticipantes,$pdo);
$aleatorioId = new NumeroAleatorioId($numParticipantes);
$numAleatorio = $aleatorio->gerar();
$numAleatorioId = $aleatorioId->gerar();

for($i=0; $i<$numParticipantes;$i++){
    $modal->update($numAleatorio[$i],$numAleatorioId[$i]);
    echo "Participante[".$numAleatorioId[$i]."] = ".$numAleatorio[$i]."<br>";
}
